...
## The owner, walked by the dog on a leash. Presentation only for Sprint 01:
## follows the dog, carries the bag, reacts with speech bubbles. No AI/combat.

@export var dog: Node2D
## Owner starts walking when the leash is longer than this.
@export var slack_length: float = 90.0
## Leash never stretches beyond this.
...
func _update_leash() -> void:
	var hand := global_position + Vector2(14.0 * _visual.scale.x, -30.0)
	var collar := dog.global_position
	var sag := clampf(1.0 - hand.distance_to(collar) / max_length, 0.0, 1.0) * 40.0
